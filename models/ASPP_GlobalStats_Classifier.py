...
class ASPP_GlobStat_Classifier(nn.Module):
    """
    ...
    def __init__(self,input_channels, output_channels, img_size, mode='multi',  batch_size=16, aspp_atrous_rates=[12, 24, 36], backbone="convslayers", bb_layers=1, bb_levels=3, dropout_frac=0.3):
        super(ASPP_GlobStat_Classifier, self).__init__()
        ...
        # === MLP Conv ===
        self.mlp_conv = ResFC_gs(256, int(img_size/2**(bb_levels)), dropout_frac)

        # === MLP Global stats ===
        self.mlp_gs = MLP_GlobStats(30)

        # === MLP Merge ===
        self.mlp_merge = MLP_Merge(32, output_channels)


    def forward(self, x, y):
        x = self.backbone(x) # B x 64 x N/8 x N/8 
        x = self.aspp(x) # B x 256 x N/8 x N/8
        x = self.mlp_conv(x) # B x O
        y = self.mlp_gs(y)
        z = self.mlp_merge(x, y)
...
if __name__ == "__main__":
    ...
    #tests model
    model = ASPP_GlobStat_Classifier(input_channels=input_channels,
