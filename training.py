...
def train_epoch(
    modes, dataloaders, optimizer, criterion, model, scheduler, dataset_sizes
):
    """
    ...
    for phase in modes:
        ...
        for _, data in tqdm(
            ...
            # record logs for samples analysis
        ...
        epoch_precision[phase] = precision_score(
            y_true=target_tot[phase],
            y_pred=pred_tot[phase],
            average='macro',
            zero_division=0.0,
        )
        epoch_recall[phase] = recall_score(
            target_tot[phase], pred_tot[phase], average='macro', zero_division=0.0
        )
        epoch_f1[phase] = f1_score(
            target_tot[phase], pred_tot[phase], average='macro', zero_division=0.0
        )
        epoch_f2[phase] = fbeta_score(
            target_tot[phase],
            pred_tot[phase],
            average='macro',
            zero_division=0.0,
        ...
        print(
            f'{phase} Loss: {epoch_losses[phase]:.4f} Acc: {epoch_accs[phase]:.4f} F1: {epoch_f1[phase]:.4f} F2: {epoch_f2[phase]:.4f} F05: {epoch_f05[phase]:.4f} Precision: {epoch_precision[phase]:.4f} Recall: {epoch_recall[phase]:.4f}'
        )
        ...
        samp_res_logs[phase]['egid'] = [
            x for row in samp_res_logs[phase]['egid'] for x in row
        ]
        samp_res_logs[phase]['target'] = [
            x for row in samp_res_logs[phase]['target'] for x in row
        ]
        samp_res_logs[phase]['pred'] = [
            x for row in samp_res_logs[phase]['pred'] for x in row
        ]
        samp_res_logs[phase]['pred_conf'] = [
            x for row in samp_res_logs[phase]['pred_conf'] for x in row
        ]
...
def train_model(
    ...
    if from_pretrained:
        assert os.path.exists(pretrained_src)
        checkpoint = torch.load(pretrained_src, weights_only=False)
    ...
    for epoch in range(first_epoch, num_epochs):
        ...
        current_metrics = {
            ...
            'F2': epoch_f2['val'],
            'F05': epoch_f05['val'],
            'Precision': epoch_precision['val'],
            'Recall': epoch_recall['val'],
        }
        ...
        if epoch == num_epochs - 1:  # save last model
            ...
            torch.save(
                ...
                os.path.join(dirs['models_dir'], "model_last.tar"),
            )
        ...
        # save samples res
        with open(
            os.path.join(dirs['log_dir'], "samp_logs.pickle"), 'wb'
        ) as output_file:
            pickle.dump(dict_samp_logs, output_file)

        # save confusion matrices
        with open(
            os.path.join(dirs['log_dir'], "confusion_matrices.pickle"), 'wb'
        ) as file:
    ...
    pd.DataFrame(
        [
            [metric, best_metrics[metric], best_epochs[metric]]
            for metric in best_epochs.keys()
        ],
        columns=['metric', 'best_val', 'epoch'],
    ).to_csv(os.path.join(dirs['log_dir'], "best_results.csv"), sep=';', index=False)

    # save metrics
    df_metrics = {}
    for phase in modes:
        df_metrics[phase] = pd.DataFrame(
            metrics[phase],
            columns=["epoch", "loss", "OA", "f1", "f2", "f05", "precision", "recall"],
        )
        df_metrics[phase].to_csv(
            os.path.join(dirs['log_dir'], f"metrics_{phase}.csv"), index=False, sep=";"
        )
    ...
    return dict_conf_mats, df_metrics, best_metrics, best_epochs, dict_samp_logs
...
def train(cfg: DictConfig):
    ...
    # change current dir
    WORKING_DIRECTORY = cfg['training']['working_directory']
    ...
    PRETRAINED_SRC = INPUTS['pretrained_src']
    ...
    #   _show infos about the datasets
    print(
        f"Datasets sizes:\n\
    ...
    for i in list(set(vals['train'])):
        print(
            f"""Label {str(i)}
            on training set: {vals['train'].count(i)}
            on validation set: {vals['val'].count(i)}
            {'-' * 10}"""
        )

    # Create model
    input_channels, img_size, _ = datasets['val'][0]['image'].size()
    model = (
        ASPP_Classifier(
    ...
    class_weights = torch.tensor(
        weights, dtype=torch.float, device=torch.device(DEVICE)
    )
    ...
    # Training
    dict_conf_mats, df_metrics, best_metrics, best_epochs, dict_logs = train_model(
    ...
    show_log_train(
        res_logs,
        os.path.join(dirs['images_dir'], 'acc_loss_evolution.png'),
        True,
        False,
    )
    ...
    show_precision_recall(
        df_metrics['val'], os.path.join(dirs['images_dir'], "precision_recall_logs.png")
    )

    # Confusion matrices for each metric
    class_labels = (
        pd.read_csv(os.path.join(DATASET_ROOT, 'class_names.csv'), sep=';')
    ...
    # Uncertainty plot
    show_uncertainty(
        dict_preds_conf=dict_logs,
        target_src=os.path.join(dirs['images_dir'], "uncertainty.png"),
        do_save=True,
        do_show=False,
        )
