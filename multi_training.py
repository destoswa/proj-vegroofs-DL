...
mode = "combinatory"  # choose in {'combination', 'combinatory'}

do_preprocess = True

params = {
        "training.parameters.batch_size": [6,8],
      }
...
def multi_training(mode, do_preprocess, params):
    ...
    dirs['single_trainings_dir'] = os.path.join(
        dirs['resroot_dir'], 'single_trainings/'
    )
    ...
    if mode == "combinatory":
        ...
        subprocess.run(lst_command)  # run the multi-training
    elif mode == "combination":
        # prepare the list to run
        for combination in params:
            ...
            lst_command.append("do_preprocess=" + str(do_preprocess))
            lst_command.append(
                "training.outputs.res_dir=" + dirs['single_trainings_dir']
            )
            lst_command.append("training.training_mode=multi")
    ...
    for x in sorted(os.listdir(dirs['single_trainings_dir'])):
        best_res_dir = os.path.join(
            os.path.join(dirs['single_trainings_dir'], x), "logs", "best_results.csv"
        )
    ...
    for metric in lst_metrics:
        ...
        #  _converts the numerical variables to corresponding types
        for var in lst_vars:
            try:
        ...
        #   _sort and save
        best_results[metric] = (
            best_results[metric].sort_values(lst_vars).reset_index(drop=True)
        )  # sorts the data with respect to the variables
        best_results[metric].to_csv(
            os.path.join(dirs['best_results_dir'], f'best_{metric}.csv'),
            sep=';',
            index=False,
        )
    ...
    if mode == 'combination':
        for metric in lst_metrics:
            show_multi_combinations(
                target_src=os.path.join(
                    dirs['images_dir'], f"combination_results_{metric}.png"
                ),
        ...
        if len(params) == 1:
            ...
            for metric in lst_metrics:
                ...
                data_all_metrics = pd.concat(
                    [
                        data_all_metrics.dropna(axis=1, how='all'),
                        best_results[metric].dropna(axis=1, how='all'),
                    ],
                    ignore_index=True,
                )
                show_one_var_combinatory(
                    target_src=os.path.join(
                        dirs['images_dir'], f"one_var_results_{metric}.png"
                    ),
                    data=best_results[metric],
            ...
            show_one_var_combinatory_all_metrics(
                target_src=os.path.join(
                    dirs['images_dir'], f"one_var_results_all_metrics.png"
                ),
            ...
            for metric in lst_metrics:
                show_grid_search(
                    target_src=os.path.join(
                        dirs['images_dir'], f"grid_search_{metric}.png"
                    ),
                    data=best_results[metric],
